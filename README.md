...
当前可用版本：`v1.0.55`。该版本已在实际环境中稳定运行：成功调用 GPU/QSV 转码、启动前功能自检通过、源片色彩参数自动保留、队列按入队时间共用 300 秒稳定等待、应用升级后旧版残留进程会被自动清理、已转码文件不再重复入队等待、数据库自动清理过期记录并压缩、启动时清理旧版残留临时文件和日志、达到重试上限的损坏文件会进入问题文件列表并被静默跳过。
...
`v1.0.52`：入队前检查数据库，已成功转码的文件不再被加入等待队列，避免每次扫描都白白等 300 秒。启动时自动清理数据库中文件已不存在的过期记录，控制数据库体积。修复 `convert_video` 中 ffmpeg 可用性检查使用裸 `'ffmpeg'` 而非 `_ffmpeg_binary()` 的问题，在 ffmpeg 不在 PATH 中的 fnOS 环境下不再误报"未安装"。

`v1.0.53`：启动时自动清理旧版残留的临时转码文件（`*_tmp_*`）和 ffmpeg/ffprobe 日志（超过 5 分钟的），并通过 `VACUUM` 压缩数据库文件体积。升级安装后自动清除过去版本产生的垃圾文件，同时保留已转码记录，不会导致重复转码。

`v1.0.54`：修复"已失败 3 次的文件被反复扫描刷日志"的缺陷。达到重试上限的文件会被持久化到独立的 `problem_files` 表，后续扫描静默跳过，不再无限刷屏。损坏文件（ffprobe 无法读取）会进入 Web UI 的"问题文件"面板，可一键"重新检测"（修复文件后重试）或"删除文件"（永久删除残缺副本）。

`v1.0.55`：核查修复上一版本引入的次生问题。UI"重新检测"现在真正生效——转码进程会感知数据库记录被清除并复位内存状态，文件在下一轮扫描（60 秒内）自动重新入队；问题文件的"重试次数"现在显示真实累计失败次数而非 1；失败原因在每次转码开始时重置，不再携带上一个文件的陈旧原因；扫描跳过检查合并为单次数据库连接，减少一半连接开销；问题文件面板打开时自动清理文件已不存在的记录。
...
- 入队前查数据库，已转码文件不进入等待队列
- 启动时清理数据库中文件已不存在的过期记录
- 启动时清理旧版残留临时转码文件和 ffmpeg/ffprobe 日志
- 数据库 VACUUM 压缩，控制数据库文件体积
- 达重试上限的损坏文件持久化到问题文件表，扫描时静默跳过不刷日志
- Web UI 问题文件面板：查看损坏文件、一键重新检测或删除
- 转码后更小时才替换原文件
- 同路径替换前先备份，失败时保留原文件
- 启动时清理旧版本遗留进程
- 启动日志打印当前转码核心版本
...
- **升级无残留**：启动时会清理旧版本遗留的转码进程和临时文件，装新版后看到的就是新版的行为。
- **数据库自维护**：启动时自动清理文件已不存在的过期记录，通过 VACUUM 压缩数据库体积，不需要用户手动维护。
...
| 版本 | 说明 |
|---|---|
| v1.0.55 | 核查修复次生问题：重新检测真正生效；重试次数显示真实累计值；失败原因不再串文件；扫描跳过合并单连接；问题面板自清理 |
| v1.0.54 | 修复失败文件反复刷日志；问题文件持久化并静默跳过；Web UI 新增问题文件面板（重新检测/删除损坏文件） |
| v1.0.53 | 启动时清理旧版残留临时文件和日志；数据库 VACUUM 压缩；升级后自动清除垃圾文件且保留已转码记录 |
| v1.0.52 | 入队前查数据库跳过已转码文件；启动时清理数据库过期记录；修复 ffmpeg 可用性检查使用裸路径 |
