...
当前可用版本：`v1.0.54`。该版本已在实际环境中稳定运行：成功调用 GPU/QSV 转码、启动前功能自检通过、源片色彩参数自动保留、队列按入队时间共用 300 秒稳定等待、应用升级后旧版残留进程会被自动清理、已转码文件不再重复入队等待、数据库自动清理过期记录并压缩、启动时清理旧版残留临时文件和日志、达到重试上限的损坏文件会进入问题文件列表并被静默跳过。
