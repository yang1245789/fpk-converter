...
"""
飞牛视频转码 Web 服务
依赖: flask + watchdog (位于 packages/ 目录，通过 cmd/main 的 PYTHONPATH 加载)
"""
import os, sys

# 确保 packages/ 目录在 Python 路径中（双重保险）
_pkg_dir = os.path.join(os.environ.get('TRIM_PKG', '/app'), 'app', 'fpkconverter', 'packages')
...
DEFAULT_CFG = {
    ...
    'container': 'mp4', 'preset': 'medium', 'threads': 1,
    'use_gpu': True, 'enabled': False
}
    ...
    if os.path.exists(CONFIG_PATH):
        try:
            ...
            if isinstance(d, dict):
                cfg = {**DEFAULT_CFG, **{k: v for k, v in d.items() if k in ALLOWED}}
        except:
            pass
...
def api_config():
    if request.method == 'POST':
        ...
        for k, v in data.items():
            ...
            if k == 'monitor_dir':
                vs = str(v)
                if vs.startswith('/') and '..' not in vs:
                    cfg[k] = vs
            elif k == 'crf':
...
def api_start():
    ...
    with open(jp, 'w') as f:
        json.dump({
            'db_path': DB_PATH, 'code_dir': CODE_DIR, 'monitor_dir': md,
...
sys.path.insert(0,c["code_dir"]);pk=j(c["code_dir"],"packages")
...
vc=VideoConverter(db,c["crf"],c["codec"],c["container"],
    c["preset"],c["threads"],c["use_gpu"])
FolderMonitor(c["monitor_dir"],vc).start()''')
    os.chmod(sc, 0o755)

    proc = subprocess.Popen([sys.executable, sc], cwd=VAR_DIR,
...
def api_logs():
    try:
        ...
        rows = db.execute(
            'SELECT * FROM processed_files ORDER BY processed_at DESC LIMIT 100'
        ).fetchall()
        db.close()
    except:
        rows = []
